...
export async function getShippingHistory(params?: DateRange) {
  const t = await getTranslations("getShippingHistoryFunction");
  ...
  try {
    ...
    throw new Error(error.message || t("fallbackError") || "Failed to fetch shipping history");
  }
}
